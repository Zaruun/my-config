# Quick links

Requirements:

- git
- wget
- nodejs & tree-sitter-cli ( global installation )

### Install NeoVim 0.8.0 in Debian/Ubuntu

```
sh -c "$(wget https://raw.githubusercontent.com/Zaruun/my-config/main/scripts/nvim/deb-install-nvim-8.sh -O -)"
```

### Install Advance Configuration

```
sh -c "$(wget https://raw.githubusercontent.com/Zaruun/my-config/main/scripts/nvim/nvim-install-config.sh -O -)"
```

### Install NodeJS and tree-sitter-cli

```
sh -c "$(wget https://raw.githubusercontent.com/Zaruun/my-config/main/scripts/nvim/node-js-tree-sitter-cli-install.sh -O -)"
```
